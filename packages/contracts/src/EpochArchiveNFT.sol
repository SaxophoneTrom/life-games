...
contract EpochArchiveNFT is ERC1155, Ownable, ReentrancyGuard {
    ...
    // ===========================================
    /**
     * @dev Returns contract name for ERC-1155
     */
    function name() public pure returns (string memory) {
        return "Game of Life on BASE Epoch";
    }

    /**
     * @dev Returns contract symbol for ERC-1155
     */
    ...
    /**
     * @dev Update epoch URIs (owner only)
     * Used for migrating storage to different providers (e.g., CloudFlare)
     * @param epochId The epoch ID to update
     * @param _artifactURI New MP4 video URI
     * @param _metadataURI New JSON metadata URI
     * @param _contributorsCID New contributors list CID
     */
    function setEpochURIs(
        uint256 epochId,
        string calldata _artifactURI,
        string calldata _metadataURI,
        string calldata _contributorsCID
    ) external onlyOwner {
        require(epochs[epochId].revealed, "EpochArchiveNFT: epoch not revealed");

        Epoch storage epoch = epochs[epochId];
        epoch.artifactURI = _artifactURI;
        epoch.metadataURI = _metadataURI;
        epoch.contributorsCID = _contributorsCID;

        emit EpochURIsUpdated(epochId, _artifactURI, _metadataURI, _contributorsCID);
    }
    ...
    /**
     * @dev Get multiple epochs with pagination (newest first)
     * @param offset Number of epochs to skip (0 = start from newest)
     * @param limit Maximum number of epochs to return
     * @return epochIds Array of epoch IDs
     * @return epochList Array of Epoch structs
     * @return total Total number of minted epochs
     */
    function getEpochs(
        uint256 offset,
        uint256 limit
        ...
        total = lastMintedEpochId;

        if (total == 0 || offset >= total) {
            return (new uint256[](0), new Epoch[](0), total);
        }
        ...
        epochList = new Epoch[](count);

        // Return newest first (descending order by epochId)
        for (uint256 i = 0; i < count; i++) {
            uint256 epochId = total - offset - i;
            epochIds[i] = epochId;
            epochList[i] = epochs[epochId];
        }

        return (epochIds, epochList, total);
    }
