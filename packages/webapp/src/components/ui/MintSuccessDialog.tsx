...
export function MintSuccessDialog({
  ...
  const handleShareFarcaster = () => {
    const text = tokenId
      ? `I just minted Infinite Life Segment #${tokenId}! Check out my creation.`
      : 'I just minted an Infinite Life Segment! Check out my creation.';
    const url = tokenId
  ...
  // BASEAPPでシェア (Hey.xyz)
  const handleShareBaseApp = () => {
    const text = tokenId
      ? `I just minted Infinite Life Segment #${tokenId}!`
      : 'I just minted an Infinite Life Segment!';
    const url = tokenId
      ? `${window.location.origin}/segment/${tokenId}`
      : window.location.origin;
    // Hey.xyz (Lens Protocol) share
    const shareUrl = `https://hey.xyz/?text=${encodeURIComponent(text)}&url=${encodeURIComponent(url)}`;
    window.open(shareUrl, '_blank');
  };
