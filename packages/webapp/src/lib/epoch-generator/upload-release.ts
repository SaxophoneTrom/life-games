...
async function uploadAsset(
  ...
  if (existingAsset) {
    ...
    if (!deleteResponse.ok && deleteResponse.status !== 204) {
      console.warn(`Failed to delete existing asset: ${await deleteResponse.text()}`);
    }
  }
 ...
 */
export async function uploadEpochVideo(
  epochId: number,
  gifBuffer: Buffer
): Promise<string> {
...
// /**
//  * Epoch動画（MP4）をGitHub Releaseにアップロード（将来用）
//  *
//  * @param epochId Epoch ID
//  * @param videoBuffer MP4動画のBuffer
//  * @returns アップロードされた動画のURL
//  */
// export async function uploadEpochVideoMP4(
//   epochId: number,
//   videoBuffer: Buffer
// ): Promise<string> {
//   const tagName = `epoch-${epochId}`;
//   const filename = `epoch-${epochId}.mp4`;
//
//   console.log(`Uploading MP4 to GitHub Release: ${tagName}/${filename}`);
//
//   const release = await getOrCreateRelease(tagName);
//   const url = await uploadAsset(release, filename, videoBuffer, 'video/mp4');
//
//   console.log(`MP4 uploaded: ${url}`);
//   return url;
// }
