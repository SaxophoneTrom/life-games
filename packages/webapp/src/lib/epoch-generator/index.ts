...
export async function generateEpoch(dryRun = false): Promise<EpochGenerationResult | null> {
  ...
  const { frames, endState, contributorInfos, usedContributions, endGeneration, startBlock, endBlock } =
  ...
  if (dryRun) {
    console.log('Dry run mode: Skipping GIF generation, upload, and mint');
    return null;
  }

  // 8. GIF生成
  console.log('Generating GIF...');
  const gifBuffer = await generateGif(frames);

  // 9. アップロード
  console.log('Uploading to GitHub Release...');
  const artifactURI = await uploadEpochGif(nextEpochId, gifBuffer);
...
export async function main() {
  ...
  try {
    ...
    if (result) {
      ...
      console.log(`GIF Size: ${result.gifBuffer.length} bytes`);
