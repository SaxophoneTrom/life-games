...
export default function BuyContent() {
  ...
  // ダイアログ表示用のtokenId（成功時に保持）
  const [mintedTokenId, setMintedTokenId] = useState<number | undefined>(undefined);

  // 手数料表示（コントラクトから取得した値を使用）
  const displayPrice = fee !== undefined ? formatEther(fee) + ' ETH' : 'Loading...';

  // Mint成功時にダイアログ表示
  useEffect(() => {
    if (isSuccess && txHash) {
      // 成功時のセル・世代数・tokenIdを保持してダイアログ表示
      setMintedCells([...injectedCells]);
      setMintedGenerations(nGenerations);
      setMintedTokenId(tokenId);
      setShowSuccessDialog(true);
    }
  }, [isSuccess, txHash, injectedCells, nGenerations, tokenId]);

  // ダイアログを閉じた時の処理
  const handleCloseSuccessDialog = () => {
    setShowSuccessDialog(false);
    reset();
    // tokenIdがあれば個別ページへ、なければギャラリーへ
    if (mintedTokenId !== undefined) {
      router.push(`/segment/${mintedTokenId}`);
    } else {
      router.push('/gallery');
    }
  };
  ...
  const handleSecretAction = () => {
    const logoCell = generateBaseLogoCells();
    setInjectedCells(logoCell);
  };
